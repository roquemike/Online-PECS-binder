...
def PECS():
    ...
    if request.args.get("cat") and request.args.get("cat") != "all":
        ...
        try:
            PECS_db = db.execute("SELECT id, name, desc FROM img OUTER JOIN tag ON image_id = id WHERE tag = :tag AND (user_id = 1 OR user_id = :user_id) ORDER BY name", user_id = session['user_id'], tag = cat)
        except:
            PECS_db = db.execute("SELECT id, name, desc FROM img OUTER JOIN tag ON image_id = id WHERE (tag = :tag AND user_id = 1) ORDER BY name", tag = cat)
        ...
        try:
            PECS_db = db.execute("SELECT id, name, desc FROM img OUTER JOIN tag ON image_id = id WHERE user_id = 1 OR user_id = :user_id ORDER BY tag", user_id = session['user_id'])
        except:
            PECS_db = db.execute("SELECT id, name, desc FROM img OUTER JOIN tag ON image_id = id WHERE user_id = 1 ORDER BY tag")
...
def edit():
    ...
    if request.method == "POST":
        ...
        if request.form.get('del'):
            ...
            PECS_db = db.execute("SELECT id, name, desc, tag FROM img OUTER JOIN tag ON image_id = id WHERE user_id = :user_id ORDER BY id", user_id = session['user_id'])
        ...
        PECS_db = db.execute("SELECT id, name, desc, tag FROM img OUTER JOIN tag ON image_id = id WHERE user_id = :user_id AND tag != :tag ORDER BY id", user_id = session['user_id'], tag="favourites")
...
def fave():
    ...
    if request.method == "POST":
        ...
        images = []
        tags = []
        PECS_db = db.execute("SELECT id, name, desc FROM img OUTER JOIN tag ON image_id = id WHERE (user_id = 1 OR user_id = :user_id) AND tag != :fav  ORDER BY name", user_id = session['user_id'], fav="favourites")
        tag_db = db.execute("SELECT id FROM img OUTER JOIN tag ON image_id = id WHERE (user_id = 1 OR user_id = :user_id) AND tag = :fav", user_id = session['user_id'], fav="favourites")
